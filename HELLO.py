...
@st.cache_data
def load_data():
    df = pd.read_csv("nifty50_historical_data.csv")  # Ensure correct file name
...
st.sidebar.header("📊 Select Investment Parameters")
selected_stock = st.sidebar.selectbox("Choose a Stock", df['Symbol'].unique())
...
# ------------------- Stock Price Trend -------------------
st.title(f"📈 {selected_stock} Stock Performance")
